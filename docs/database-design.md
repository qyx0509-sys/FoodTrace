...
| `provider_poi_id` | `varchar(128)` | 是 | 腾讯 POI 标识；手动店铺为空 |
...
- `CHECK ((source = 'TENCENT_POI' AND provider_poi_id IS NOT NULL) OR source = 'MANUAL')`；
- `UNIQUE (user_id, source, provider_poi_id)`；PostgreSQL 允许多个空值，因此手动店铺不受误限制；
- `UNIQUE (id, user_id)`，为复合外键提供目标；
...
- `UNIQUE (id, user_id)`；
- 复合外键 `(store_id, user_id) REFERENCES stores(id, user_id) ON DELETE CASCADE`；
...
约束：复合外键 `(record_id, user_id)` 指向 `food_records` 并 `ON DELETE CASCADE`；`UNIQUE (user_id, record_id, type, normalized_name)`；`CHECK (sort_order >= 0)`。
...
约束：`UNIQUE (user_id, normalized_name)`、`UNIQUE (id, user_id)`、`CHECK (color IS NULL OR color ~ '^#[0-9A-Fa-f]{6}$')`。
...
- `(record_id, user_id)` 到 `food_records(id, user_id)` 的级联外键；
- `(tag_id, user_id)` 到 `tags(id, user_id)` 的级联外键；
...
约束：复合外键 `(record_id, user_id)` 级联删除；`UNIQUE (object_key)`；`UNIQUE (record_id, sort_order)`；大小为 1 至 10 MB；宽高为正数或空。服务层保证单条记录不超过 9 张。
...
约束：复合外键 `(record_id, user_id)` 级联删除；`UNIQUE (pending_object_key)`；`INDEX (user_id, status, expires_at)`。
...
- 父子表同时声明 `UNIQUE (id, user_id)` 和复合外键，防止跨用户错误关联。
