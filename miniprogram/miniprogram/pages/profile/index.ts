...
interface ProfileCustomOptions {
  load(): Promise<void>;
  onLogout(): void;
  onOpenRecords(): void;
  onRetry(): void;
...
Page<ProfileData, ProfileCustomOptions>({
  ...
  onOpenRecords(): void {
    void wx.navigateTo({ url: '/pages/records/index' });
  },
  onLogout(): void {
    void wx.showModal({
      cancelText: '取消',
      confirmColor: '#F05A47',
      confirmText: '退出',
      content: '本地登录状态会被清除，需要重新登录才能查看私人记录。',
      title: '退出登录？',
      success: async (result) => {
        if (!result.confirm) return;
        const app = getApp<{ globalData: FoodTraceGlobalData }>();
        try {
          await new UserService(createApiClient(app.globalData.apiBaseUrl)).logout();
        } catch {
          /* 本地会话仍需清除。 */
        }
        authSession.clear();
